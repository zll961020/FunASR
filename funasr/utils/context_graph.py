...
from funasr.tokenizer.whisper_tokenizer import WhisperTokenizer, SenseVoiceTokenizer
...
def tokenize(context_list_path, symbol_table, bpe_model=None, tokenizer=None):
    ...
    for context_txt in context_txts:
        ...
        tokens = []
        if bpe_model is not None:
            tokens = context_txt #tokenize_by_bpe_model(sp, context_txt)
        else:
            for ch in context_txt:
                if ch == ' ':
                    ch = "▁"
                tokens.append(ch)
        if tokenizer is not None and (isinstance(tokenizer, WhisperTokenizer) or isinstance(tokenizer, SenseVoiceTokenizer)):
            ids = tokenizer.encode(context_txt)
        ...
        while queue:
            ...
            for tok, nxt in node.next.items():
                if isinstance(self.tokenizer, WhisperTokenizer):
                    # Whisper: 用 detokenize 把单个 id 反解为字符 组合的id才有结果 还是显示token id
                    tok_lbl = str(tok)
                else:
